defmodule StatsWeb.UploadControllerTest do
  ...
  describe "upload" do
    test "valid stats data will retrun 201", %{conn: conn} do
      conn = post(conn, Routes.upload_path(conn, :upload), %{"stats" => valid_stats_data()})
      assert response(conn, 204)
    end

    test "will return a 400 when attempting to upload duplicate data", %{conn: conn} do
      valid_stats_data() |> UploadParser.Football.parse() |> Football.store_stats()
